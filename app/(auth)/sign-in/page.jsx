...
const SignInForm = () => {
  ...
  const handleCredentialsSubmit = async (e) => {
    ...
    setError(null);
    try {
      // First authenticate via the backend proxy API to set the authToken cookie and get clear error messages
      ...
      // Then trigger NextAuth to establish the frontend session
  ...
  const handleOAuthSignIn = (provider) => {
    setError(`Sign in with ${provider} is not supported by the backend yet. Please use email and password.`);
  };
  ...
  return (
    <div className="auth-card w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl relative z-10 mx-auto">
      ...
      {isRegistered && !error && (
        <div className="animate-item mb-4 p-3.5 bg-emerald-950/40 border border-emerald-850/50 rounded-xl flex items-start gap-2.5 text-emerald-200 text-xs">
          <svg className="size-4 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
      ...
      <form onSubmit={handleCredentialsSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            ...
            disabled={isLoading}
            className="animate-item bg-white/5 border border-white/10 hover:bg-white/10 disabled:opacity-50 text-white font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2.5 group cursor-pointer disabled:cursor-not-allowed text-xs"
          >
          ...
          <button
            ...
            className="animate-item bg-white/5 border border-white/10 hover:bg-white/10 disabled:opacity-50 text-white font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2.5 group cursor-pointer disabled:cursor-not-allowed text-xs"
          >
            <svg className="size-4 transition-transform group-hover:scale-110" fill="currentColor" viewBox="0 0 24 24">
